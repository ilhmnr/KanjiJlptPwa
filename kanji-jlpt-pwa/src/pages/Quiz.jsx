...
// Bangun daftar soal: tiap soal punya 1 jawaban benar + 3 pengecoh acak
function buildQuestions(data, mode) {
  ...
  return pool.map((correct) => {
    ...
    return {
      ...
      prompt: mode === 'kanji-to-arti' ? correct.kanji : correct.arti,
      options: options.map((o) => ({
        id: o.id,
        label: mode === 'kanji-to-arti' ? o.arti : o.kanji
      }))
    };
  });
}
...
export default function Quiz() {
  ...
  const mode = params.get('mode') || 'kanji-to-arti';
  ...
  const handleAnswer = useCallback((optionId) => {
    if (selected) return; // sudah menjawab soal ini
  ...
  if (questions.length === 0) {
    return (
      <div className="page">
        ...
        <div className="empty-state">Data kanji tidak cukup untuk quiz.</div>
      </div>
    );
  }
  ...
  if (finished) {
    ...
    return (
      <div className="page quiz-result">
        <PageHeader title="Hasil Quiz" showBack={false} />
        <div className="card quiz-result-card">
          ...
          <div className="flex gap-12 mt-24" style={{ width: '100%' }}>
            <button className="btn btn-outline" style={{ flex: 1 }} onClick={() => navigate(`/quiz/${level}`)}>
              Ganti Mode
            </button>
  ...
  return (
    <div className="page">
      ...
      <div className="card quiz-question-card">
        <div className={`quiz-prompt ${mode === 'kanji-to-arti' ? 'font-jp' : ''}`}>{question.prompt}</div>
      </div>
      ...
      <div className="quiz-options mt-16">
        {question.options.map((opt) => {
          ...
          return (
            <button
              key={opt.id}
              className={`card card-tappable quiz-option ${state} ${mode === 'arti-to-kanji' ? 'font-jp' : ''}`}
